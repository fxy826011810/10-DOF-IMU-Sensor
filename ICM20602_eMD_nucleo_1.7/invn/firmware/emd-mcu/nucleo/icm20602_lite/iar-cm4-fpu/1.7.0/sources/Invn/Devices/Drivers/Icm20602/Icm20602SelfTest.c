...
#if (MEMS_CHIP != HW_ICM20609)
static int save_settings(struct inv_icm20602 * s, struct recover_regs * saved_regs)
{
	int result = 0;

	result |= inv_icm20602_mems_read_reg(s, MPUREG_PWR_MGMT_1, 1, &saved_regs->pwr_mgmt_1);

	/* wake up */
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_PWR_MGMT_1, CLK_SEL);

	result |= inv_icm20602_mems_read_reg(s, MPUREG_INT_ENABLE, 1, &saved_regs->int_enable);
	result |= inv_icm20602_mems_read_reg(s, MPUREG_FIFO_EN, 1, &saved_regs->fifo_en);
	result |= inv_icm20602_mems_read_reg(s, MPUREG_USER_CTRL, 1, &saved_regs->user_ctrl);
	result |= inv_icm20602_mems_read_reg(s, MPUREG_CONFIG, 1, &saved_regs->config);
	result |= inv_icm20602_mems_read_reg(s, MPUREG_GYRO_CONFIG, 1, &saved_regs->gyro_config);
	result |= inv_icm20602_mems_read_reg(s, MPUREG_ACCEL_CONFIG, 1, &saved_regs->accel_config);
	result |= inv_icm20602_mems_read_reg(s, MPUREG_ACCEL_CONFIG_2, 1, &saved_regs->accel_config_2);
	result |= inv_icm20602_mems_read_reg(s, MPUREG_SMPLRT_DIV, 1, &saved_regs->smplrt_div);
	result |= inv_icm20602_mems_read_reg(s, MPUREG_LP_CONFIG, 1, &saved_regs->lp_mode);
	result |= inv_icm20602_mems_read_reg(s, MPUREG_PWR_MGMT_2, 1, &saved_regs->pwr_mgmt_2);
...
static int recover_settings(struct inv_icm20602 * s, const struct recover_regs * saved_regs)
{
	int result = 0;

	// Stop sensors
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_PWR_MGMT_2, BIT_PWR_ALL_OFF);

	// Restore sensor configurations
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_INT_ENABLE, saved_regs->int_enable);
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_FIFO_EN, saved_regs->fifo_en);
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_USER_CTRL, saved_regs->user_ctrl);
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_CONFIG, saved_regs->config);
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_GYRO_CONFIG, saved_regs->gyro_config);
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_ACCEL_CONFIG, saved_regs->accel_config);
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_ACCEL_CONFIG_2, saved_regs->accel_config_2);
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_SMPLRT_DIV, saved_regs->smplrt_div);
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_LP_CONFIG, saved_regs->lp_mode);
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_PWR_MGMT_1, saved_regs->pwr_mgmt_1);
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_PWR_MGMT_2, saved_regs->pwr_mgmt_2);
...
*/
static int check_gyro_self_test(struct inv_icm20602 * s, int *meanNormalTestValues, int *meanSelfTestValues)
{
	int ret_val = 0;
	...
	result = inv_icm20602_mems_read_reg(s, MPUREG_SELF_TEST_X_GYRO, 3, regs);
	if (result)
		return result;
...
*  @return zero as success. A non-zero return value indicates failure in self test.
*/
static int check_accel_self_test(struct inv_icm20602 * s, int *meanNormalTestValues, int *meanSelfTestValues)
{
	int ret_val = 0;
	...
	result = inv_icm20602_mems_read_reg(s, MPUREG_SELF_TEST_X_ACCEL, 3, regs);
	if (result)
		return result;
...
*/
static int do_test(struct inv_icm20602 * s, int self_test_flag, int *gyro_result, int *accel_result, int lp_mode)
{
	int result = 0;
	...
	if (lp_mode == ST_GYRO_LP_MODE)
		...
		d = BIT_PWR_GYRO_STBY;
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_PWR_MGMT_2, d);
	inv_icm20602_sleep(GYRO_ENGINE_UP_TIME);
	...
	/* clear signal path */
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_USER_CTRL, 1);

	inv_icm20602_sleep(30);

	/* disable interrupt */
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_INT_ENABLE, 0);

	/* disable the sensor output to FIFO */
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_FIFO_EN, 0);

	/* disable fifo reading */
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_USER_CTRL, 0);

	/* setup parameters */
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_CONFIG, DEF_ST_GYRO_LPF);
	...
	if (lp_mode == ST_GYRO_LP_MODE)
		...
		d = 0;
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_LP_CONFIG, d);
	...
	if (lp_mode == ST_ACCEL_LP_MODE)
		...
		d |= DEF_ST_ACCEL_LPF;
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_ACCEL_CONFIG_2, d);

	if (lp_mode)
		result |= inv_icm20602_mems_write_reg_one(s, MPUREG_SMPLRT_DIV, DEF_SELFTEST_SAMPLE_RATE_LP);
	else
		result |= inv_icm20602_mems_write_reg_one(s, MPUREG_SMPLRT_DIV, DEF_SELFTEST_SAMPLE_RATE);

	/* wait for the sampling rate change to stabilize */
	inv_icm20602_sleep(GYRO_ENGINE_UP_TIME);

	d = self_test_flag | DEF_SELFTEST_GYRO_FS;
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_GYRO_CONFIG, d);

	d = self_test_flag | DEF_SELFTEST_ACCEL_FS;
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_ACCEL_CONFIG, d);

	/* wait for the output to get stable */
	inv_icm20602_sleep(DEF_ST_STABLE_TIME);
	...
	while(ss < 200) {
		/* clear FIFO */
		result |= inv_icm20602_mems_write_reg_one(s, MPUREG_USER_CTRL, BIT_FIFO_RST);

		/* enable FIFO reading */
		result |= inv_icm20602_mems_write_reg_one(s, MPUREG_USER_CTRL, BIT_FIFO_EN);
		...
		result |= inv_icm20602_mems_write_reg_one(s, MPUREG_FIFO_EN, d);

		result |= inv_icm20602_mems_write_reg_one(s, MPUREG_PWR_MGMT_1, dd);

		if (lp_mode)
			inv_icm20602_sleep(DEF_GYRO_WAIT_TIME_LP);
		else
			inv_icm20602_sleep(DEF_GYRO_WAIT_TIME);
...
#if ((MEMS_CHIP != HW_ICM20602) && (MEMS_CHIP != HW_ICM20603))
		result |= inv_icm20602_mems_write_reg_one(s, MPUREG_FIFO_EN, 0); // stop FIFO
#endif
		result |= inv_icm20602_mems_read_reg(s, MPUREG_FIFO_COUNTH, FIFO_COUNT_BYTE, data);
		...
		while ((i < packet_count) && (ss < 200)) {
			short vals[3];
			result |= inv_icm20602_mems_read_reg(s, MPUREG_FIFO_R_W, packet_size, data);
			ind = 0;
...
int inv_icm20602_run_selftest(struct inv_icm20602 * s)
{
#if ((MEMS_CHIP != HW_ICM20690) && (MEMS_CHIP != HW_ICM20602) && (MEMS_CHIP != HW_ICM20603) && (MEMS_CHIP != HW_ICM20789))
	...
	/* enable sensros */
	result |= inv_icm20602_mems_write_reg_one(s, MPUREG_PWR_MGMT_2, 0);
	inv_icm20602_sleep(GYRO_ENGINE_UP_TIME);
	...
	compass_result = !inv_icm20602_check_akm_self_test(s);
#endif
test_fail:
...
void inv_icm20602_get_st_bias(struct inv_icm20602 * s, int * st_bias)
{
	int axis, axis_sign;
	int gravity;
	int i, t;
...
void inv_icm20602_set_st_bias(struct inv_icm20602 * s, int * st_bias)
{
	int i, t;
	int scale;
	int check_gyro = 0;
